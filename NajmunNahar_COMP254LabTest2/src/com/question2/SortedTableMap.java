...
public class SortedTableMap<K,V> extends AbstractSortedMap<K,V> {
  ...
  {
          int j = findIndex(key);
          if (j < size() && compare(key, table.get(j)) == 0)         // key already exists
          { 
        	System.out.println("Key Already Exists! "+key+","+value.toString()+ " Entry was not added!");
            return null;
          } else 
        	table.add(j, new MapEntry<K,V>(key,value));             	// otherwise add new entry
            System.out.println(key+","+value.toString()+" Entry Added Succesfully!");
            return null;         
  }
  ...
  public static void main(String[] args)
  {
        ...
        map.put(1, "A");
        map.put(3, "C");
        map.putOnlyIfAbsent(3, "D");  //entry will not be added as key 3 already exists
        map.putOnlyIfAbsent(2, "B");  //entry will be added as key does not exist
